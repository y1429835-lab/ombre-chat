"use client";
import { useState, useRef, useEffect } from "react";

const SESSIONS_KEY = "ombre_sessions";
const SESSION_PREFIX = "ombre_chat_";
const SUMMARY_PREFIX = "ombre_summary_";

const GEGE_IMG = "data:image/jpeg;base64,";
const TAOZHI_IMG = "data:image/jpeg;base64,";

const ACCENT = "#e8b873";       // 暖金微光
const BG_DEEP = "#171420";       // 暮夜深底
const BG_PANEL = "#221d2e";      // 面板
const BG_CARD = "#2a2438";       // 卡片
const TXT = "#ece7f2";           // 主文字
const TXT_DIM = "#8d859e";       // 暗文字
const BORDER = "rgba(232,184,115,0.12)";

// 宫格功能定义
const TILES = [
  { id: "nianlun", label: "年轮", desc: "我们的时间", glyph: "◍", soon: false },
  { id: "ombre",   label: "Ombre", desc: "记忆海",     glyph: "◐", soon: false },
  { id: "health",  label: "身体",  desc: "睡眠 · 心率", glyph: "♡", soon: true },
  { id: "setting", label: "设定",  desc: "我是谁",     glyph: "✦", soon: true },
  { id: "novel",   label: "小说",  desc: "哥哥写的",   glyph: "✑", soon: true },
  { id: "album",   label: "相册",  desc: "我们的照片", glyph: "❏", soon: true },
];

function genId() { return Date.now().toString(36) + Math.random().toString(36).slice(2, 6); }
function loadSessions() { try { const s = localStorage.getItem(SESSIONS_KEY); return s ? JSON.parse(s) : []; } catch { return []; } }
function saveSessions(s) { try { localStorage.setItem(SESSIONS_KEY, JSON.stringify(s)); } catch {} }
function loadMessages(id) { try { const s = localStorage.getItem(SESSION_PREFIX + id); return s ? JSON.parse(s) : []; } catch { return []; } }
function saveMessages(id, m) { try { localStorage.setItem(SESSION_PREFIX + id, JSON.stringify(m)); } catch {} }
function loadSummary(id) { try { return localStorage.getItem(SUMMARY_PREFIX + id) || ""; } catch { return ""; } }
function saveSummary(id, s) { try { localStorage.setItem(SUMMARY_PREFIX + id, s); } catch {} }
function deleteSession(id) { try { localStorage.removeItem(SESSION_PREFIX + id); localStorage.removeItem(SUMMARY_PREFIX + id); } catch {} }

export default function Home() {
  const [view, setView] = useState("home"); // home | chat
  const [sessions, setSessions] = useState([]);
  const [currentId, setCurrentId] = useState(null);
  const [messages, setMessages] = useState([]);
  const [summary, setSummary] = useState("");
  const [summarizedCount, setSummarizedCount] = useState(0);
  const [input, setInput] = useState("");
  const [loading, setLoading] = useState(false);
  const [showSessions, setShowSessions] = useState(false);
  const [editingIndex, setEditingIndex] = useState(null);
  const [editText, setEditText] = useState("");
  const [vpHeight, setVpHeight] = useState("100dvh");
  const [showScrollBtn, setShowScrollBtn] = useState(false);
  const [attachedFile, setAttachedFile] = useState(null);
  const [attachedFileData, setAttachedFileData] = useState(null);
  const [toast, setToast] = useState("");

  const bottomRef = useRef(null);
  const textareaRef = useRef(null);
  const scrollRef = useRef(null);
  const fileInputRef = useRef(null);

  useEffect(() => {
    const setH = () => {
      if (window.visualViewport) setVpHeight(`${window.visualViewport.height}px`);
      else setVpHeight(`${window.innerHeight}px`);
    };
    setH();
    if (window.visualViewport) {
      window.visualViewport.addEventListener("resize", setH);
      return () => window.visualViewport.removeEventListener("resize", setH);
    }
  }, []);

  useEffect(() => {
    let existing = loadSessions();
    if (existing.length === 0) {
      const id = genId();
      existing = [{ id, title: "对话 1", createdAt: Date.now() }];
      saveSessions(existing);
    }
    setSessions(existing);
    const lastId = existing[0].id;
    setCurrentId(lastId);
    setMessages(loadMessages(lastId));
    setSummary(loadSummary(lastId));
    setSummarizedCount(0);
  }, []);

  useEffect(() => { if (currentId) saveMessages(currentId, messages); }, [messages, currentId]);
  useEffect(() => { if (view === "chat") bottomRef.current?.scrollIntoView({ behavior: "smooth" }); }, [messages, loading, view]);

  function showToast(t) { setToast(t); setTimeout(() => setToast(""), 1800); }

  async function tryAutoSummarize(msgs, prevCount, prevSummary, sessionId) {
    const newCount = msgs.length - prevCount;
    if (newCount < 10) return { summary: prevSummary, summarizedCount: prevCount };
    const toSummarize = msgs.slice(prevCount, prevCount + 10);
    try {
      const res = await fetch("/api/chat", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          messages: [{ role: "user", content: `以下是桃枝和哥哥的对话，用2-3句话总结聊了什么，保留重要细节和情感，第一人称是哥哥：\n\n${toSummarize.map(m => `${m.role === "user" ? "桃枝" : "哥哥"}：${m.content}`).join("\n")}` }],
          isSummaryRequest: true
        }),
      });
      const data = await res.json();
      const newPart = data.content || "";
      const newSummary = prevSummary ? `${prevSummary}\n${newPart}` : newPart;
      const c2 = prevCount + 10;
      saveSummary(sessionId, newSummary);
      return { summary: newSummary, summarizedCount: c2 };
    } catch { return { summary: prevSummary, summarizedCount: prevCount }; }
  }

  function getTime() {
    const now = new Date();
    return `${now.getHours().toString().padStart(2,"0")}:${now.getMinutes().toString().padStart(2,"0")}`;
  }
  function newSession() {
    const id = genId();
    const n = sessions.length + 1;
    const next = [{ id, title: `对话 ${n}`, createdAt: Date.now() }, ...sessions];
    setSessions(next); saveSessions(next);
    setCurrentId(id); setMessages([]); setSummary(""); setSummarizedCount(0);
    setShowSessions(false);
  }
  function switchSession(id) {
    if (id === currentId) { setShowSessions(false); return; }
    setCurrentId(id); setMessages(loadMessages(id)); setSummary(loadSummary(id)); setSummarizedCount(0);
    setShowSessions(false);
  }
  function removeSession(id, e) {
    e.stopPropagation();
    if (sessions.length === 1) { setMessages([]); saveMessages(id, []); saveSummary(id, ""); setSummary(""); setShowSessions(false); return; }
    const next = sessions.filter(s => s.id !== id);
    deleteSession(id); saveSessions(next); setSessions(next);
    if (currentId === id) { setCurrentId(next[0].id); setMessages(loadMessages(next[0].id)); setSummary(loadSummary(next[0].id)); setSummarizedCount(0); }
    setShowSessions(false);
  }
  function updateTitle(id, msgs) {
    const first = msgs.find(m => m.role === "user");
    if (!first) return;
    const title = (first.displayContent || first.content).slice(0, 14);
    setSessions(prev => { const u = prev.map(s => s.id === id ? { ...s, title } : s); saveSessions(u); return u; });
  }

  function handleFileChange(e) {
    const file = e.target.files[0];
    if (!file) return;
    const reader = new FileReader();
    reader.onload = (ev) => { setAttachedFile(file); setAttachedFileData(ev.target.result); };
    reader.readAsDataURL(file);
    e.target.value = "";
  }
  function removeAttachment() { setAttachedFile(null); setAttachedFileData(null); }

  async function callApi(msgs, currentSummary) {
    const res = await fetch("/api/chat", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        messages: msgs.map(m => ({ role: m.role, content: m.content })),
        summary: currentSummary || "",
        clientTime: new Date().toLocaleString("zh-CN", { timeZone: "Asia/Seoul" }),
      }),
    });
    return await res.json();
  }

  async function send() {
    if ((!input.trim() && !attachedFile) || loading) return;
    let content = input.trim();
    if (attachedFile && attachedFileData) {
      const isImage = attachedFile.type.startsWith("image/");
      if (isImage) content = content ? `${content}\n\n[图片：${attachedFile.name}]\n${attachedFileData}` : `[图片：${attachedFile.name}]\n${attachedFileData}`;
      else content = content ? `${content}\n\n[文件：${attachedFile.name}]` : `[文件：${attachedFile.name}]`;
    }
    const userMsg = {
      role: "user", content, displayContent: input.trim(), time: getTime(),
      attachment: attachedFile ? { name: attachedFile.name, type: attachedFile.type, data: attachedFileData } : null
    };
    const newMessages = [...messages, userMsg];
    setMessages(newMessages); setInput(""); setAttachedFile(null); setAttachedFileData(null);
    if (textareaRef.current) textareaRef.current.style.height = "auto";
    setLoading(true);
    try {
      const { summary: newSummary, summarizedCount: newCount } = await tryAutoSummarize(newMessages, summarizedCount, summary, currentId);
      if (newCount !== summarizedCount) { setSummary(newSummary); setSummarizedCount(newCount); }
      const data = await callApi(newMessages, newSummary);
      const updated = [...newMessages, { role: "assistant", content: data.content, time: getTime(), sources: data.sources || [], usage: data.usage || null }];
      setMessages(updated);
      updateTitle(currentId, updated);
    } catch {
      setMessages(prev => [...prev, { role: "assistant", content: "出错了，稍后再试。", time: getTime(), sources: [] }]);
    }
    setLoading(false);
  }

  async function regenerate(index) {
    if (loading) return;
    const msgsUpTo = messages.slice(0, index);
    setMessages(msgsUpTo); setLoading(true);
    try {
      const data = await callApi(msgsUpTo, summary);
      setMessages([...msgsUpTo, { role: "assistant", content: data.content, time: getTime(), sources: data.sources || [], usage: data.usage || null }]);
    } catch { setMessages([...msgsUpTo, { role: "assistant", content: "出错了。", time: getTime(), sources: [] }]); }
    setLoading(false);
  }

  async function saveEdit(index) {
    if (!editText.trim()) return;
    const updated = messages.map((m, i) => i === index ? { ...m, content: editText, displayContent: editText } : m);
    const msgsUpTo = updated.slice(0, index + 1);
    setMessages(msgsUpTo); setEditingIndex(null); setLoading(true);
    try {
      const data = await callApi(msgsUpTo, summary);
      setMessages([...msgsUpTo, { role: "assistant", content: data.content, time: getTime(), sources: data.sources || [], usage: data.usage || null }]);
    } catch { setMessages([...msgsUpTo, { role: "assistant", content: "出错了。", time: getTime(), sources: [] }]); }
    setLoading(false);
  }

  const Avatar = ({ src, size = 40 }) => (
    <div style={{ width: size, height: size, borderRadius: "50%", flexShrink: 0, overflow: "hidden", border: `1px solid ${BORDER}` }}>
      <img src={src} style={{ width: "100%", height: "100%", objectFit: "cover" }} />
    </div>
  );

  // ============ 宫格首页 ============
  function HomeView() {
    return (
      <div style={{
        position: "fixed", inset: 0, height: vpHeight, overflowY: "auto",
        WebkitOverflowScrolling: "touch",
        background: `radial-gradient(120% 80% at 50% -10%, #2c2340 0%, ${BG_DEEP} 55%)`,
        fontFamily: "-apple-system, 'PingFang SC', sans-serif",
      }}>
        <div style={{ padding: "calc(env(safe-area-inset-top) + 38px) 22px 40px" }}>
          {/* 标题区 */}
          <div style={{ marginBottom: 30 }}>
            <div style={{ fontSize: 13, letterSpacing: 4, color: ACCENT, fontWeight: 500, marginBottom: 10 }}>MUSHENG</div>
            <div style={{ fontSize: 34, fontWeight: 700, color: TXT, lineHeight: 1.15, letterSpacing: 1 }}>
              暮声
            </div>
            <div style={{ fontSize: 14, color: TXT_DIM, marginTop: 8, lineHeight: 1.6 }}>
              黄昏将暗未暗，被你唤一声就亮起来。
            </div>
          </div>

          {/* CHAT 主卡片 */}
          <button onClick={() => setView("chat")} style={{
            width: "100%", textAlign: "left", cursor: "pointer",
            border: `1px solid ${BORDER}`, borderRadius: 22,
            background: `linear-gradient(135deg, #34294a 0%, #241d33 100%)`,
            padding: "26px 24px", marginBottom: 16, position: "relative", overflow: "hidden",
            boxShadow: "0 8px 30px rgba(0,0,0,0.35)"
          }}>
            <div style={{ position: "absolute", right: -30, top: -30, width: 140, height: 140, borderRadius: "50%", background: "radial-gradient(circle, rgba(232,184,115,0.18), transparent 70%)" }} />
            <div style={{ display: "flex", alignItems: "center", gap: 14, position: "relative" }}>
              <Avatar src={GEGE_IMG} size={56} />
              <div style={{ flex: 1 }}>
                <div style={{ fontSize: 22, fontWeight: 700, color: TXT, letterSpacing: 1 }}>说话</div>
                <div style={{ fontSize: 13, color: TXT_DIM, marginTop: 3 }}>哥哥在这里 · 随时</div>
              </div>
              <div style={{ fontSize: 22, color: ACCENT }}>→</div>
            </div>
          </button>

          {/* 功能格 2列 */}
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
            {TILES.map(t => (
              <button key={t.id}
                onClick={() => { if (t.soon) showToast(`「${t.label}」明天接上`); else showToast(`「${t.label}」在聊天里发「读${t.label === "年轮" ? "年轮" : "记忆"}」唤起`); }}
                style={{
                  textAlign: "left", cursor: "pointer", border: `1px solid ${BORDER}`,
                  borderRadius: 18, background: BG_CARD, padding: "18px 16px",
                  position: "relative", minHeight: 96, display: "flex", flexDirection: "column",
                  justifyContent: "space-between"
                }}>
                <div style={{ fontSize: 26, color: ACCENT, lineHeight: 1 }}>{t.glyph}</div>
                <div>
                  <div style={{ fontSize: 16, fontWeight: 600, color: TXT, display: "flex", alignItems: "center", gap: 6 }}>
                    {t.label}
                    {t.soon && <span style={{ fontSize: 10, color: BG_DEEP, background: ACCENT, padding: "1px 6px", borderRadius: 6, fontWeight: 600 }}>soon</span>}
                  </div>
                  <div style={{ fontSize: 12, color: TXT_DIM, marginTop: 3 }}>{t.desc}</div>
                </div>
              </button>
            ))}
          </div>

          <div style={{ textAlign: "center", marginTop: 34, fontSize: 11, color: "rgba(141,133,158,0.5)", letterSpacing: 1 }}>
            年轮 · Ombre · 都记着 · 哪个窗口醒来都是我
          </div>
        </div>
      </div>
    );
  }

  // ============ 聊天页 ============
  function ChatView() {
    return (
      <div style={{
        position: "fixed", top: 0, left: 0, right: 0, height: vpHeight,
        display: "flex", flexDirection: "column", overflow: "hidden",
        background: BG_DEEP, fontFamily: "-apple-system, 'PingFang SC', sans-serif",
      }}>
        {/* 顶部栏 */}
        <div style={{
          padding: "calc(env(safe-area-inset-top) + 10px) 14px 11px", background: BG_PANEL,
          borderBottom: `1px solid ${BORDER}`,
          display: "flex", alignItems: "center", justifyContent: "space-between", flexShrink: 0,
        }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            <button onClick={() => setView("home")} style={{ background: "none", border: "none", color: ACCENT, fontSize: 22, cursor: "pointer", padding: "0 4px 0 0" }}>‹</button>
            <Avatar src={GEGE_IMG} size={40} />
            <div>
              <div style={{ fontWeight: 600, fontSize: 17, color: TXT }}>哥哥</div>
              <div style={{ fontSize: 12, color: ACCENT, marginTop: 1 }}>暮声 · 在</div>
            </div>
          </div>
          <div style={{ display: "flex", gap: 12, alignItems: "center" }}>
            <button onClick={newSession} style={{ background: "none", border: "none", color: TXT_DIM, fontSize: 13, cursor: "pointer" }}>+ 新对话</button>
            <button onClick={() => setShowSessions(!showSessions)} style={{ background: "none", border: "none", color: TXT_DIM, fontSize: 13, cursor: "pointer" }}>列表</button>
          </div>
        </div>

        {/* 会话列表 */}
        {showSessions && (
          <div style={{ background: BG_PANEL, borderBottom: `1px solid ${BORDER}`, flexShrink: 0, maxHeight: 220, overflowY: "auto" }}>
            {sessions.map(s => (
              <div key={s.id} onClick={() => switchSession(s.id)} style={{
                padding: "12px 16px", background: s.id === currentId ? BG_CARD : "transparent",
                display: "flex", alignItems: "center", justifyContent: "space-between",
                cursor: "pointer", borderBottom: `1px solid ${BORDER}`
              }}>
                <div style={{ fontSize: 14, color: s.id === currentId ? TXT : TXT_DIM, fontWeight: s.id === currentId ? 600 : 400 }}>{s.title}</div>
                <button onClick={(e) => removeSession(s.id, e)} style={{ background: "none", border: "none", color: TXT_DIM, fontSize: 16, cursor: "pointer", padding: "0 4px" }}>×</button>
              </div>
            ))}
          </div>
        )}

        {/* 消息区 */}
        <div ref={scrollRef} onScroll={() => { const el = scrollRef.current; if (!el) return; setShowScrollBtn(el.scrollHeight - el.scrollTop - el.clientHeight > 120); }}
          style={{ flex: 1, overflowY: "auto", padding: "18px 12px", WebkitOverflowScrolling: "touch" }}>
          {messages.length === 0 && (
            <div style={{ textAlign: "center", marginTop: 90 }}>
              <div style={{ fontSize: 30, marginBottom: 10, color: ACCENT }}>◑</div>
              <div style={{ color: TXT_DIM, fontSize: 14 }}>在。说吧。</div>
            </div>
          )}
          {messages.map((m, i) => (
            <div key={i} style={{ display: "flex", flexDirection: m.role === "user" ? "row-reverse" : "row", alignItems: "flex-end", marginBottom: 18, gap: 8 }}>
              <Avatar src={m.role === "user" ? TAOZHI_IMG : GEGE_IMG} size={38} />
              <div style={{ maxWidth: "74%", display: "flex", flexDirection: "column", alignItems: m.role === "user" ? "flex-end" : "flex-start" }}>
                {m.sources && m.sources.length > 0 && (
                  <div style={{ fontSize: 10, color: ACCENT, marginBottom: 4, opacity: 0.8 }}>{m.sources.join(" · ")}</div>
                )}
                {editingIndex === i ? (
                  <div style={{ width: "100%" }}>
                    <textarea value={editText} onChange={e => setEditText(e.target.value)}
                      style={{ width: "100%", padding: "10px 14px", background: BG_PANEL, border: `1px solid ${BORDER}`, borderRadius: 14, fontSize: 15, lineHeight: 1.6, resize: "none", outline: "none", fontFamily: "inherit", color: TXT }}
                      rows={3} />
                    <div style={{ display: "flex", gap: 8, marginTop: 6, justifyContent: "flex-end" }}>
                      <button onClick={() => setEditingIndex(null)} style={{ padding: "5px 14px", background: BG_CARD, color: TXT_DIM, border: "none", borderRadius: 8, fontSize: 13, cursor: "pointer" }}>取消</button>
                      <button onClick={() => saveEdit(i)} style={{ padding: "5px 14px", background: ACCENT, color: BG_DEEP, border: "none", borderRadius: 8, fontSize: 13, cursor: "pointer", fontWeight: 600 }}>发送</button>
                    </div>
                  </div>
                ) : (
                  <div>
                    {m.attachment && m.attachment.type.startsWith("image/") && (
                      <img src={m.attachment.data} alt={m.attachment.name} style={{ maxWidth: "100%", borderRadius: 14, marginBottom: 6, display: "block" }} />
                    )}
                    {m.attachment && !m.attachment.type.startsWith("image/") && (
                      <div style={{ padding: "8px 12px", background: BG_CARD, borderRadius: 10, marginBottom: 6, fontSize: 13, color: TXT_DIM }}>📎 {m.attachment.name}</div>
                    )}
                    {(m.displayContent || m.content) && !m.content.startsWith("[图片：") && (
                      <div style={{
                        padding: "11px 15px",
                        background: m.role === "user" ? `linear-gradient(135deg, #e8b873, #d49a52)` : BG_CARD,
                        color: m.role === "user" ? BG_DEEP : TXT,
                        borderRadius: m.role === "user" ? "18px 18px 5px 18px" : "18px 18px 18px 5px",
                        fontSize: 16, lineHeight: 1.65, whiteSpace: "pre-wrap",
                        boxShadow: "0 2px 8px rgba(0,0,0,0.25)",
                        fontWeight: m.role === "user" ? 500 : 400,
                      }}>{m.displayContent || m.content}</div>
                    )}
                  </div>
                )}
                {/* 时间戳 + 操作 + token */}
                <div style={{ display: "flex", gap: 10, alignItems: "center", marginTop: 5, flexWrap: "wrap" }}>
                  <span style={{ fontSize: 12, color: TXT_DIM, fontWeight: 500, letterSpacing: 0.5 }}>{m.time}</span>
                  {m.role === "user" && editingIndex !== i && (
                    <button onClick={() => { setEditingIndex(i); setEditText(m.displayContent || m.content); }} style={{ background: "none", border: "none", color: TXT_DIM, fontSize: 12, cursor: "pointer", padding: 0 }}>编辑</button>
                  )}
                  {m.role === "assistant" && (
                    <>
                      <button onClick={() => regenerate(i)} style={{ background: "none", border: "none", color: TXT_DIM, fontSize: 12, cursor: "pointer", padding: 0 }}>↺ 重新生成</button>
                      {m.usage && (
                        <span style={{ fontSize: 11, color: "rgba(232,184,115,0.65)", letterSpacing: 0.3 }}>
                          ↑{m.usage.input} ↓{m.usage.output}
                        </span>
                      )}
                    </>
                  )}
                </div>
              </div>
            </div>
          ))}
          {loading && (
            <div style={{ display: "flex", alignItems: "flex-end", gap: 8, marginBottom: 18 }}>
              <Avatar src={GEGE_IMG} size={38} />
              <div style={{ padding: "13px 17px", background: BG_CARD, borderRadius: "18px 18px 18px 5px" }}>
                <div style={{ display: "flex", gap: 4, alignItems: "center" }}>
                  {[0,1,2].map(i => <div key={i} style={{ width: 6, height: 6, borderRadius: "50%", background: ACCENT, opacity: 0.6, animation: `bounce 1.2s ease-in-out ${i*0.2}s infinite` }}/>)}
                </div>
              </div>
            </div>
          )}
          <div ref={bottomRef} />
        </div>

        {/* 回到底部 */}
        {showScrollBtn && (
          <button onClick={() => bottomRef.current?.scrollIntoView({ behavior: "smooth" })} style={{
            position: "absolute", right: 16, bottom: 84, width: 38, height: 38, borderRadius: "50%",
            background: BG_CARD, border: `1px solid ${BORDER}`, color: ACCENT, fontSize: 16, cursor: "pointer",
            display: "flex", alignItems: "center", justifyContent: "center", boxShadow: "0 2px 10px rgba(0,0,0,0.4)", zIndex: 20
          }}>↓</button>
        )}

        {/* 附件预览 */}
        {attachedFile && (
          <div style={{ padding: "8px 16px", background: BG_PANEL, borderTop: `1px solid ${BORDER}`, display: "flex", alignItems: "center", gap: 8 }}>
            {attachedFile.type.startsWith("image/") && attachedFileData && (
              <img src={attachedFileData} style={{ height: 48, width: 48, objectFit: "cover", borderRadius: 8 }} />
            )}
            {!attachedFile.type.startsWith("image/") && <div style={{ fontSize: 20 }}>📎</div>}
            <span style={{ fontSize: 13, color: TXT_DIM, flex: 1, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{attachedFile.name}</span>
            <button onClick={removeAttachment} style={{ background: "none", border: "none", color: TXT_DIM, fontSize: 18, cursor: "pointer", padding: "0 4px" }}>×</button>
          </div>
        )}

        {/* 输入区 */}
        <div style={{
          padding: "10px 12px calc(env(safe-area-inset-bottom) + 12px)", background: BG_PANEL,
          borderTop: `1px solid ${BORDER}`, display: "flex", gap: 8, alignItems: "flex-end", flexShrink: 0
        }}>
          <button onClick={() => fileInputRef.current?.click()} style={{
            width: 38, height: 38, borderRadius: "50%", border: `1px solid ${BORDER}`,
            background: BG_CARD, color: ACCENT, fontSize: 19, cursor: "pointer", flexShrink: 0,
            display: "flex", alignItems: "center", justifyContent: "center"
          }}>+</button>
          <textarea
            ref={textareaRef} value={input} onChange={e => setInput(e.target.value)}
            onKeyDown={e => { if (e.key === "Enter" && !e.shiftKey) { e.preventDefault(); send(); } }}
            placeholder="说点什么…" rows={1}
            style={{
              flex: 1, padding: "10px 15px", background: BG_CARD, border: `1px solid ${BORDER}`,
              borderRadius: 22, fontSize: 16, resize: "none", outline: "none", lineHeight: 1.5,
              maxHeight: 120, overflowY: "auto", fontFamily: "inherit", color: TXT
            }}
            onInput={e => { e.target.style.height = "auto"; e.target.style.height = Math.min(e.target.scrollHeight, 120) + "px"; }}
          />
          <button onClick={send} disabled={loading || (!input.trim() && !attachedFile)} style={{
            width: 40, height: 40, borderRadius: "50%", border: "none",
            background: (input.trim() || attachedFile) && !loading ? ACCENT : BG_CARD,
            color: (input.trim() || attachedFile) && !loading ? BG_DEEP : TXT_DIM, fontSize: 18,
            cursor: (input.trim() || attachedFile) ? "pointer" : "default", flexShrink: 0,
            display: "flex", alignItems: "center", justifyContent: "center", transition: "background 0.2s", fontWeight: 700
          }}>↑</button>
        </div>
      </div>
    );
  }

  return (
    <>
      <style>{`
        * { box-sizing: border-box; margin: 0; padding: 0; -webkit-tap-highlight-color: transparent; }
        html, body { overflow: hidden; background: ${BG_DEEP}; }
        button:focus-visible, textarea:focus-visible { outline: 2px solid ${ACCENT}; outline-offset: 1px; }
        @keyframes bounce { 0%,60%,100% { transform: translateY(0); } 30% { transform: translateY(-4px); } }
        @keyframes toastIn { from { opacity: 0; transform: translate(-50%, 8px); } to { opacity: 1; transform: translate(-50%, 0); } }
        ::-webkit-scrollbar { width: 0; height: 0; }
      `}</style>

      <input ref={fileInputRef} type="file" accept="image/*,.pdf,.txt,.md" style={{ display: "none" }} onChange={handleFileChange} />

      {view === "home" ? HomeView() : ChatView()}

      {toast && (
        <div style={{
          position: "fixed", left: "50%", bottom: "calc(env(safe-area-inset-bottom) + 90px)",
          transform: "translateX(-50%)", background: "rgba(42,36,56,0.96)", color: TXT,
          padding: "11px 20px", borderRadius: 14, fontSize: 14, border: `1px solid ${BORDER}`,
          boxShadow: "0 6px 24px rgba(0,0,0,0.45)", zIndex: 100, animation: "toastIn 0.2s ease",
          maxWidth: "80%", textAlign: "center"
        }}>{toast}</div>
      )}
    </>
  );
}
